...
# Portable sed in-place edit (Alpine uses busybox sed which supports -i)
sed_inplace() {
    sed -i "$@"
}
...
if grep -qi "^\[pi${SYSMODEL}\]" $FILE; then
    sed_inplace "/^\[pi${SYSMODEL}\]/a dtparam=i2c_arm=on" $FILE && echo "DONE";
else
